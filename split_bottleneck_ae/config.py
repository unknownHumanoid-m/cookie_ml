...
MODEL = {
    ...
    # Pool op inside TimeAdaptivePoolAdapter.
    #   "adaptive" -> F.adaptive_avg_pool1d(row_mean, pool_len). Handy for
    #                 training but hls4ml/FPGA toolchains reject the op.
    #   "fixed"    -> nn.AvgPool1d(kernel=W//pool_len, stride=W//pool_len).
    #                 Deterministic kernel/stride; what the FPGA path wants.
    # Overridable at launch time via env: SPLIT_AE_POOL_TYPE=fixed|adaptive.
    "pool_type": os.environ.get("SPLIT_AE_POOL_TYPE", "fixed"),
...
IO = {
    ...
    # Overridable via SPLIT_AE_RUN_NAME so the fixed-pool variant lands in
    # its own directory without clobbering the adaptive-pool baseline.
    # Default points at the fixed-pool reproduction run so an sbatch eval
    # against the original fixedpool model.pth writes its summary here
    # instead of overwriting the reference run.
    "run_name": os.environ.get(
        "SPLIT_AE_RUN_NAME", "split_bottleneck_ae_v1_fixedpool1d_repro"
    ),

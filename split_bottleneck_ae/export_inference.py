...
DEFAULT_EXPORTS_DIR = os.path.join(_PROJ_DIR, "exports")
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default=None,
                        help="Where to write the exported subgraph. Defaults "
                             "to split_bottleneck_ae/exports/<run_name>/, "
                             "keeping every export next to the code and "
                             "namespaced so runs don't collide.")
    ...
    out_dir = args.out_dir or os.path.join(
        DEFAULT_EXPORTS_DIR, cfg.IO["run_name"]
    )
    os.makedirs(out_dir, exist_ok=True)
    print(f"[export] writing to {out_dir}")
    ...
    torch.save({
        ...
        "arch": {
            ...
            "adapter": (
                f"time_pool_type={cfg.MODEL.get('pool_type', 'adaptive')}"
                f"_len={cfg.MODEL['adapter_pool_len']}"
            ),
            "pool_type": cfg.MODEL.get("pool_type", "adaptive"),
    ...
    with open(meta_path, "w") as f:
        json.dump({
            ...
            "bottleneck_count_dim": model.bottleneck_count_dim,
            "bottleneck_phase_dim": model.bottleneck_phase_dim,
            "adapter_pool_len": cfg.MODEL["adapter_pool_len"],
            "pool_type": cfg.MODEL.get("pool_type", "adaptive"),
            "min_pulses": cfg.DATA["min_pulses"],
            "max_pulses": cfg.DATA["max_pulses"],
        }, f, indent=2)

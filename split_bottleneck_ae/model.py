...
class TimeAdaptivePoolAdapter(nn.Module):
    ...
    def __init__(
        ...
        if self.pool_type == "fixed":
            if input_width is None:
                raise ValueError(
                    "TimeAdaptivePoolAdapter: pool_type='fixed' needs "
                    "input_width (the encoder's W); got None."
                )
            kernel = int(input_width) // int(pool_len)
            if kernel < 1:
                raise ValueError(
                    f"TimeAdaptivePoolAdapter: input_width={input_width} "
                    f"< pool_len={pool_len}; fixed AvgPool1d needs W >= L."
                )
            self.fixed_pool = nn.AvgPool1d(kernel_size=kernel, stride=kernel)
            self._kernel = kernel
            ...
            raise ValueError(
                f"TimeAdaptivePoolAdapter: unknown pool_type={pool_type!r} "
                "(expected 'adaptive' or 'fixed')"
            )

    def forward(self, feat: torch.Tensor) -> torch.Tensor:
        # feat: (N, C, H, W). Collapse rows, then pool along W.
        row_mean = feat.mean(dim=2)                              # (N, C, W)
        if self.fixed_pool is not None:
            pooled = self.fixed_pool(row_mean)                   # (N, C, L)
        else:
            pooled = F.adaptive_avg_pool1d(row_mean, self.pool_len)
...
def build_model_from_config(config_module) -> SplitBottleneckAE:
    ...
    return SplitBottleneckAE(
        ...
        pool_type=m.get("pool_type", "adaptive"),
    )
